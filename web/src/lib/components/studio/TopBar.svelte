...
>
  <!-- Wordmark: 2×15px accent tick + BLUE SHIFT 13.5/700 + STUDIO 10.5/500 @0.26em -->
  ...
  <!-- Breadcrumb slot -->
  <nav class="flex items-center gap-2 text-[12px]" aria-label="Breadcrumb">
    {#if breadcrumb}
      {@render breadcrumb()}
    {:else}
  ...
  >
    <span class="font-semibold text-[10.5px] tracking-[0.14em] text-text-muted">RENDER</span>
    <span class="font-mono text-[10.5px] text-text-faint">{renderState}</span>
  </div>
  ...
  <DropdownMenu>
    <DropdownMenuTrigger
      ...
      class="flex h-6 w-6 flex-none items-center justify-center rounded-full border border-border-strong bg-bg-4 font-mono text-[10.5px] text-text-muted outline-none transition-colors duration-hover ease-out hover:border-border-hover data-[state=open]:border-accent-border"
    >
      {initials}
    </DropdownMenuTrigger>
